...
portalRouter.get('/marks',asyncRoute(async(q:any,r:any)=>{const s=q.user.role==='STUDENT'?await prisma.student.findUnique({where:{userId:q.user.id}}):null;r.json(await prisma.mark.findMany({where:s?{studentId:s.id}:{},include:{student:{include:{user:true}},subject:true}}))}));
